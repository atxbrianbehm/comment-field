...
export function TransformOverlay({
  ...
  return <svg className={`transform-overlay ${overlay.locked ? "is-locked" : ""}`} width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
    <polygon points={overlay.points.map((point) => `${point.x},${point.y}`).join(" ")} />
    <line x1={overlay.center.x} y1={overlay.center.y} x2={overlay.rotationHandle.x} y2={overlay.rotationHandle.y} />
    {overlay.points.map((point, index) => <g key={index} className="transform-handle scale-handle" transform={`translate(${point.x} ${point.y})`} onPointerDown={(event) => onBegin("scale", event)}><circle className="transform-hit" r="24" /><rect x="-6" y="-6" width="12" height="12" rx="3" /></g>)}
    <g className="transform-handle rotate-handle" transform={`translate(${overlay.rotationHandle.x} ${overlay.rotationHandle.y})`} onPointerDown={(event) => onBegin("rotate", event)}><circle className="transform-hit" r="24" /><circle r="7" /></g>
