...
describe("project schema migration", () => {
  ...
  it("preserves the implicitly hero-linked burst cue when migrating schema v16", () => {
    const legacy = createDefaultProject();
    legacy.version = 16;
    const take = legacy.takes[0];
    const cardId = legacy.compositions[0].cards[0].cardId;
    const easing = { x1: 0.16, y1: 1, x2: 0.3, y2: 1 };
    const path = { start: { x: 0, y: 0 }, control1: { x: 0, y: 0.1 }, control2: { x: 0, y: -0.08 } };
    take.population.postHeroBurstStartTime = 7;
    take.hero = {
      cardId,
      keyframes: [
        { id: "source", time: 2, value: { kind: "source" }, interpolation: "bezier", easing, holdDuration: 0, path },
        { id: "pose", time: 3.5, value: { kind: "pose", transform: { x: 0.5, y: 0.5, z: 1.4, scale: 1.7, rotation: 0 }, targetSpace: "screen", surroundingDim: 0.4, surroundingBlur: 2.5 }, interpolation: "bezier", easing, holdDuration: 0, path },
      ],
      reflowRadius: 0.34,
      attraction: 0.48,
      falloff: 1.4,
    ...
    const restored = deserializeProject(JSON.stringify(legacy));
    expect(restored.version).toBe(21);
    expect(restored.takes[0].population.postHeroBurstStartTime).toBe(3.5);
  });

  it("migrates legacy composition images into export-safe background plates", () => {
    const legacy = createDefaultProject();
    legacy.version = 17;
    const composition = legacy.compositions[0] as typeof legacy.compositions[0] & { backgroundImage?: string };
    composition.backgroundImage = "data:image/png;base64,legacy";

    const restored = deserializeProject(JSON.stringify(legacy));
    expect(restored.version).toBe(21);
    expect(restored.compositions[0].backgroundPlate).toEqual({
      source: "data:image/png;base64,legacy",
    ...
    expect("backgroundImage" in restored.compositions[0]).toBe(false);
  });
});
